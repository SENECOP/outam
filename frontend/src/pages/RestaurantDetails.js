...
const RestaurantDetails = () => {
    ...
    const fetchRestaurantDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/etablissements/restaurant/${commercantId}`,
          {
  ...
  if (!restaurant) {
    return (
      <div className="text-center p-4 text-red-500">Restaurant non trouvé</div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      ...
      <p className="text-xl text-gray-700 mb-2">
        <strong>Commerçant:</strong> {restaurant.idCommercant.nom}
      </p>
      <p className="text-xl text-gray-700 mb-4">
        <strong>Email:</strong> {restaurant.idCommercant.email}
      </p>
