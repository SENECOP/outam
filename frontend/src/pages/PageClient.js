  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="flex items-center mb-4">
        ...
        <button onClick={toggleMenuVisibility} className={`ml-4 px-4 py-2 rounded-lg ${isMenuActive ? 'bg-green-500 text-white' : 'bg-gray-300'}`}>
          {isMenuActive ? <Eye className="inline-block mr-1" /> : <EyeOff className="inline-block mr-1" />} {isMenuActive ? "Actif" : "Désactivé"}
        </button>
      </div>
      
      <div className="border p-4 rounded-lg bg-gray-100">
        <h2 className="font-medium mb-2">Partager votre QR Code</h2>
        <p className="text-sm mb-4">Vous pouvez télécharger ce QR Code et l'afficher dans votre restaurant.</p>
        
        <div className="flex flex-col items-center">
          <div className="p-4 bg-white border rounded-lg mb-4">
            <QRCodeSVG ref={qrCodeRef} value={restaurantUrl} size={150} level="H" includeMargin={true} />
          </div>
          
          <div className="flex space-x-4 mb-4">
            <button onClick={downloadPDF} className="px-4 py-2 bg-green-500 text-white rounded-lg">Télécharger PDF</button>
            <button onClick={downloadImage} className="px-4 py-2 bg-blue-500 text-white rounded-lg">Télécharger image</button>
          </div>
          
          <div className="flex space-x-4">
            <a href={`https://wa.me/?text=${encodeURIComponent(restaurantUrl)}`} target="_blank" rel="noopener noreferrer" className="text-green-500 text-2xl">
              <FaWhatsapp />
            </a>
            <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(restaurantUrl)}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 text-2xl">
              <FaFacebook />
            </a>
            <a href={`mailto:?subject=Menu&body=${encodeURIComponent(restaurantUrl)}`} className="text-red-500 text-2xl">
              <FaEnvelope />
            </a>
          </div>
        </div>
